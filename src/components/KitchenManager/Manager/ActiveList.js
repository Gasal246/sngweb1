...
function ActiveList({ handleEditClick, data, handleStatus, handleViewPackage, handleView, handleDelete }) {
    const columns = [
        {
            name: "	NAME",
...

        {
            ...
            cell: row => <span className="">
                ...
                <OverlayTrigger placement="top" overlay={<Tooltip >View Package</Tooltip>}>
                    <Link to="#" onClick={() => handleViewPackage(row)} className="btn btn-secondary btn-sm rounded-11 my-1 me-2"> <i> <svg xmlns="http://www.w3.org/2000/svg" width="16" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="feather feather-eye"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg> </i> </Link>
                </OverlayTrigger>
            </span>
        },
    ]
