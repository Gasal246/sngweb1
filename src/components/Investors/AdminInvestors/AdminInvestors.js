...
export default function AdminInvestors() {
  ...
  const updateData = async () => {
    ...
    console.log(new_formData,'asdasd00000000000000000000000000000000000000000')
  ...
  const getCampsList = async (client_id) => {
    const { data, error } = await api.get(client_id?.length ? `${baseUrl}/super-admin/api/camps?client_id=${client_id}` : `${baseUrl}/super-admin/api/camps`);
    if (data) {
      let optionList = []
  ...
  const getPlantList = async () => {
    const { data, error } = await api.get(`${baseUrl}/super-admin/api/plant-manager`);
    if (data) {
      let optionList = []
      for (let index = 0; index < data?.data?.list?.length; index++) {
  ...
  const getKitchenList = async () => {
    const { data, error } = await api.get(`${baseUrl}/super-admin/api/kitchen-manager`);
    if (data) {
      let optionList = []
      for (let index = 0; index < data?.data?.list?.length; index++) {
        let obj = { value: data?.data?.list[index]?.id, label: data?.data?.list[index]?.full_name }
  ...
  const handleAssignData = (value, name) => {
    debugger
  ...
  const assignHandle = async () => {
    ...
    if (assignType === 'camps-to-investor') {
      req.investor_id = AssignID
      req.camp_ids = ids
    }
    if (assignType === 'stores') {
      debugger
      req.investor_id = AssignID
      req.store_ids = ids
      req.commission_pct = modalData?.commission_pct
    }

    if (assignType === 'plants') {
      req.investor_id = AssignID
      req.plant_ids = ids
      req.commission_pct = modalData?.commission_pct
    }

    if (assignType === 'kitchen-managers') {
      req.investor_id = AssignID
      req.kitchen_manager_ids = ids
      req.commission_pct = modalData?.commission_pct
    }

    const { data, error } = await api.post(assignType === 'camps-to-investor' ? `${baseUrl}/super-admin/api/camps/assign-camps-to-investor` : `${baseUrl}/super-admin/api/investors/assign-${assignType}`, req);
    if (data) {
      handleCloseModal()
      toast.success(data.message ? data.message : data.messages, {
        position: toast.POSITION.TOP_RIGHT,
                        ...
                        <div className="salesdatatable">
                          <div className="table-responsive">
                            <ActiveList
                              ...
                              handleAssignCamps={(row) => handleAssign(row, 'camps-to-investor')}
                              handleAssignStores={(row) => handleAssign(row, 'stores')}
                              handleAssignPlants={(row) => handleAssign(row, 'plants')}
                              handleAssignKitchen={(row) => handleAssign(row, 'kitchen-managers')
                              }
                            />
                          </div>
                        </div>
      ...
      {assignModal && (
        <Modal size="xl" show={assignModal}>
          <form id="form" onSubmit={handleSubmit(assignHandle)} validate="true">
            <Modal.Header>
              <Modal.Title>
                {assignType === 'camps-to-investor' && 'Assign To Camps'}
                {assignType === 'stores' && 'Assign To Stores Managers'}
                {assignType === 'plants' && 'Assign To Plants Managers'}
                {assignType === 'kitchen-managers' && 'Assign To Kitchen Managers'}
                ...
                <Row className="mb-2">
                  <div className="col-lg-4 col-md-6 col-sm-12 form-group">
                    <label className="form-label">
                      {assignType === 'camps-to-investor' && 'Client'}
                      {assignType === 'stores' && 'Stores'}
                      {assignType === 'plants' && 'Plants'}
                      {assignType === 'kitchen-managers' && 'Kitchen Managers'}
                    </label>
                    {assignType === 'camps-to-investor' && < Select
                      id="campidsList"
                      placeholder="Select Client"
                      onChange={(e) => handleAssignData(e, 'client_admin')}
                      singleSelect="true"
                    ...
                    {assignType === 'kitchen-managers' && < Select
                      id="kitchenList"
                      placeholder="Select kitchen Managers"
                      onChange={(e) => handleAssignData(e)}
                      singleSelect="true"
                      options={kitchenList}
                      required={true}
                  ...
                  {assignType === 'camps-to-investor' ?
                    ...
                    :
                    <div className="col-lg-4 col-md-6 col-sm-12 form-group">
                      <label htmlFor="commission_pct" className="form-label">Commission(%)</label>
                      <input className="form-control" type="number" id="commission_pct" placeholder="Enter commission_pct" name="commission_pct"
